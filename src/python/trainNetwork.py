...
def loadData(dataset_root, data_name, input_size):
    ...
    cam_desire_valid_path = fullfile(data_root, 'cam/desire/test')
    ...
    # validation data
    cam_raw_valid = readImgsMT(cam_valid_path)
    cam_desire_valid = readImgsMT(cam_desire_valid_path)
    ...
    cam_valid = torch.where(prj_fov_mask, cam_raw_valid, torch.tensor([0.]))

    return cam_surf, cam_train, cam_valid, cam_raw_valid, cam_desire_valid, prj_train, prj_valid, mask_corners
